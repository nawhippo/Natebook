...
import SoloProject.SocialMediaApp.CustomAuthenticationProvider;
...
public class VerificationService {
    ...
    public ResponseEntity<Object> createAndSendEmailToken(String email) {
        Optional<EmailToken> existingToken = emailTokenRepository.findByEmail(email.trim());
        if (existingToken.isPresent()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of("message", "A verification token for this email already exists."));
        } else {
            String token = UUID.randomUUID().toString().substring(0, 8);
            EmailToken emailToken = new EmailToken();
            emailToken.setToken(token);
            emailToken.setEmail(email.trim());
